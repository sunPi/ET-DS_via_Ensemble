...
imp.count.plot <- ggplot(imp.counts, aes(Variable, Count)) + 
  ...
  scale_y_continuous(breaks=seq(0,150,30)) + labs(title='MEDUSA SNV - Boruta Frequency of Importance')
...
normHits.plot <- ggplot(Boruta5k.results, aes(Variable, normHits)) + 
  ...
  labs(title='MEDUSA SNV - Boruta Normalised Hits')
